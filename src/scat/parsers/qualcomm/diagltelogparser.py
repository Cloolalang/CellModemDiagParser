...
import scat.parsers.qualcomm.lte_tbs as lte_tbs
...
class DiagLteLogParser:
    def __init__(self, parent):
        ...
        self.no_process = {
            # Connected-mode neighbor meas: requested for log mask but format not yet parsed
            i(c.LOG_LTE_ML1_CONNECTED_MODE_NEIGHBOR_MEAS_REQ_RESPONSE): None,
        }
        self.process = {
            # ML1
            ...
            # MAC (0xB063/0xB064 standard; 9607 also sends 0xB167-0xB16A - use safe wrapper to avoid crash)
            ...
            i(c.LOG_LTE_ML1_MAC_RAR_MSG1_REPORT): lambda x, y, z: self.parse_lte_mac_9607_dl_safe(x, y, z),
            i(c.LOG_LTE_ML1_MAC_RAR_MSG2_REPORT): lambda x, y, z: self.parse_lte_mac_9607_ul_safe(x, y, z),
            i(c.LOG_LTE_ML1_MAC_UE_IDENTIFICATION_MESSAGE_MSG3_REPORT): lambda x, y, z: self.parse_lte_mac_9607_dl_safe(x, y, z),
            i(c.LOG_LTE_ML1_MAC_CONTENTION_RESOLUTION_MESSAGE_MSG4_REPORT): lambda x, y, z: self.parse_lte_mac_9607_ul_safe(x, y, z),
    ...
    def parse_lte_ml1_scell_meas(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        return {'stdout': 'LTE Primary Cell: EARFCN: {}, PCI: {:3d}, RSRP: {:.0f}, RSSI: {:.0f}, RSRQ: {:.0f}, priority: {}'.format(item.earfcn, pci, real_rsrp, real_rssi, real_rsrq, serv_layer_priority),
                'ts': pkt_ts}
    ...
    def parse_lte_ml1_ncell_meas(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        for i in range(n_cells):
            ...
            stdout += 'Neighbor cell {}: EARFCN: {}, PCI: {:3d}, RSRP: {:.0f}, RSSI: {:.0f}, RSRQ: {:.0f}\n'.format(i, item.earfcn, n_pci, n_real_rsrp, n_real_rssi, n_real_rsrq)
    ...
    def parse_lte_ml1_scell_meas_response_cell_v36(self, cell_id: int, cell_bytes: bytes, rsrp_offset: int=16, snr_offset: int=80, sir_cinr_offset: int=104):
        ...
        connected_line = 'LTE Primary Cell (Connected): PCI: {:3d}, DL RSRP: {:.0f}, RSSI: {:.0f}, RSRQ: {:.0f}\n'.format(pci, rsrp, rssi, rsrq)
        return (connected_line, pci, rsrp, rssi, rsrq)
                ...
                if subpkt_header.id == 0x19:
                    ...
                    if subpkt_header.version == 36:
                        ...
                        for y in range(subpkt_scell_meas_v36.num_cells):
                            cell_result = self.parse_lte_ml1_scell_meas_response_cell_v36(y, subpkt_body[pos_meas:pos_meas+128])
                            connected_line, pci, rsrp, rssi, rsrq = cell_result
                            if y == 0:
                                stdout += 'LTE Primary Cell: EARFCN: {}, PCI: {:3d}, RSRP: {:.0f}, RSSI: {:.0f}, RSRQ: {:.0f}\n'.format(
                                    subpkt_scell_meas_v36.earfcn, pci, rsrp, rssi, rsrq)
                            else:
                                stdout += connected_line
                            pos_meas += 128
                        ...
                        for y in range(subpkt_scell_meas_v48.num_cells):
                            cell_result = self.parse_lte_ml1_scell_meas_response_cell_v48(y, subpkt_body[pos_meas:pos_meas+140])
                            connected_line, pci, rsrp, rssi, rsrq = cell_result
                            if y == 0:
                                stdout += 'LTE Primary Cell: EARFCN: {}, PCI: {:3d}, RSRP: {:.0f}, RSSI: {:.0f}, RSRQ: {:.0f}\n'.format(
                                    subpkt_scell_meas_v48.earfcn, pci, rsrp, rssi, rsrq)
                            else:
                                stdout += connected_line
                            pos_meas += 140
                    ...
                    #         pos_meas += 148
                        ...
                        if self.parent:
                            logged = getattr(self.parent, '_logged_unknown_ml1_scell_versions', None)
                            if logged is not None and subpkt_header.version not in logged:
                                logged.add(subpkt_header.version)
                                self.parent.logger.log(logging.DEBUG, 'Unknown LTE ML1 Serving Cell Meas Serving Cell Measurement Result subpacket version {}'.format(subpkt_header.version))
                                self.parent.logger.log(logging.DEBUG, util.xxd(pkt_body))
                else:
                    if self.parent:
    ...
    def parse_lte_mac_subpkt_v1_rach_attempt(self, pkt_header, subpkt_hdr, subpkt_body: bytes) -> tuple[list[bytes], str | None]:
        ret = []
        kpi_line = None
        ...
        # Build KPI line (success or failure): preamble, preamble_power_dB, ta, tc_rnti, earfcn, p_max, backoff
        result_str = 'success' if subpkt_mac_rach_attempt.rach_result == 0x00 else 'failure'
        preamble_str = str(rach_msg1.preamble_index) if rach_msg1 is not None else '-'
        preamble_power_str = str(rach_msg1.preamble_power_offset) if rach_msg1 is not None else '-'
        ta_val = rach_msg2.ta if rach_msg2 is not None else None
        ta_str = '-' if (ta_val is None or ta_val == 0xFFFF) else str(ta_val)
        tc_rnti_str = '0x{:x}'.format(rach_msg2.tc_rnti) if rach_msg2 is not None else '-'
        backoff_str = str(rach_msg2.backoff) if rach_msg2 is not None else '-'
        earfcn_str = str(util.calculate_dl_earfcn(add_info.ul_earfcn)) if add_info is not None else '-'
        p_max_str = str(add_info.p_max) if add_info is not None else '-'
        kpi_line = 'LTE KPI RACH: result={}, attempt={}, contention={}, preamble={}, preamble_power_dB={}, ta={}, tc_rnti={}, earfcn={}, p_max={}, backoff={}'.format(
            result_str, subpkt_mac_rach_attempt.num_attempt, subpkt_mac_rach_attempt.contention,
            preamble_str, preamble_power_str, ta_str, tc_rnti_str, earfcn_str, p_max_str, backoff_str)

        if subpkt_mac_rach_attempt.rach_result != 0x00:  # RACH Failure, 0x00 == Success (attempt 1 failure then success is normal)
            if self.parent:
                self.parent.logger.log(logging.DEBUG, 'RACH result is not success: {}'.format(subpkt_mac_rach_attempt.rach_result))
                self.parent.logger.log(logging.DEBUG, util.xxd(subpkt_body))
            return (ret, kpi_line)
    ...
    def parse_lte_mac_subpkt_v1_dl_transport_block(self, pkt_header, subpkt_hdr, subpkt_body: bytes, args: dict = None) -> tuple[list[bytes], list[str], dict]:
        ret = []
        kpi_lines = []
        total_dl_bytes = 0
        ...
        extra = {'dl_bytes': total_dl_bytes} if total_dl_bytes else {}
        return ret, kpi_lines, extra

    def parse_lte_mac_subpkt_v1_ul_transport_block(self, pkt_header, subpkt_hdr, subpkt_body: bytes, args: dict = None) -> tuple[list[bytes], list[str], dict]:
        ret = []
        kpi_lines = []
        total_ul_bytes = 0
        ul_harq_ndi = []  # (harq_id, ndi) for ACK/NACK inference: NDI toggled => ACK, same => NACK
        ul_mcs_sum = 0
        ul_mcs_count = 0
        n_samples = subpkt_body[0]
        ...
        for j in range(n_samples):
            ...
            if isinstance(subpkt_mac_ul_tb, subpkt_mac_ul_tb_struct) or isinstance(subpkt_mac_ul_tb, subpkt_mac_ul_tb_struct_v2):
                # Grant 16-bit: try mid (bits 5-9), low (0-4), high (11-15); use first valid MCS 1-28
                grant = subpkt_mac_ul_tb.grant
                ul_mcs_mid = (grant >> 5) & 0x1F
                ul_mcs_low = grant & 0x1F
                ul_mcs_high = (grant >> 11) & 0x1F
                if 1 <= ul_mcs_mid <= 28:
                    ul_mcs = ul_mcs_mid
                elif 1 <= ul_mcs_low <= 28:
                    ul_mcs = ul_mcs_low
                elif ul_mcs_high <= 28:
                    ul_mcs = ul_mcs_high
                else:
                ...
                n_prb = self.parent.lte_last_bw_ul[radio_id] if self.parent and radio_id < len(getattr(self.parent, 'lte_last_bw_ul', [])) else 0
                ul_tbs = lte_tbs.mcs_to_tbs_bytes(ul_mcs, n_prb) if n_prb else 0
                total_ul_bytes += ul_tbs or 0
                # NDI (New Data Indicator) from grant: 1 bit, toggled => new TX (eNodeB ACK), same => retx (eNodeB NACK). Bit position is modem-dependent (--ul-ndi-bit).
                ul_mcs_sum += ul_mcs
                ul_mcs_count += 1
                ul_ndi_bit = getattr(self.parent, 'ul_ndi_bit', 6) if self.parent else 6
                ndi = (grant >> ul_ndi_bit) & 1
                ul_harq_ndi.append((subpkt_mac_ul_tb.harq_id, ndi))
        ...
        extra = {}
        if total_ul_bytes:
            extra['ul_bytes'] = total_ul_bytes
        if ul_harq_ndi:
            extra['ul_harq_ndi'] = ul_harq_ndi
        if ul_mcs_count > 0:
            extra['ul_mcs_sum'] = ul_mcs_sum
            extra['ul_mcs_count'] = ul_mcs_count
        return ret, kpi_lines, extra

    def parse_lte_mac_subpkt_v1(self, pkt_header, pkt_body: bytes, args: dict, is_dl_log: bool = True):
        """Parse MAC subpackets. is_dl_log: True when called from DL block, False from UL block.
        Bytes are attributed by log packet type: in a DL log, 0x07->dl 0x08->ul; in a UL log, 0x07->ul 0x08->dl
        (some modems use 0x07 for UL TB in UL log packets)."""
        ...
        for i in range(num_subpacket):
            ...
            if subpkt_mac.id == 0x06:  # RACH Attempt
                rach_ret, rach_kpi = self.parse_lte_mac_subpkt_v1_rach_attempt(pkt_header, subpkt_mac, subpkt_body)
                mac_pkts += rach_ret
                if rach_kpi:
                    kpi_lines.append(rach_kpi)
            elif subpkt_mac.id == 0x07:  # Transport Block (DL structure; some modems use for UL in UL log)
                dl_ret, dl_kpi, dl_extra = self.parse_lte_mac_subpkt_v1_dl_transport_block(pkt_header, subpkt_mac, subpkt_body, args)
                mac_pkts += dl_ret
                kpi_lines += dl_kpi
                n = dl_extra.get('dl_bytes', 0) or 0
                if is_dl_log:
                    result_dl_bytes += n
                else:
                    result_ul_bytes += n
            elif subpkt_mac.id == 0x08:  # UL Transport Block
                ul_ret, ul_kpi, ul_extra = self.parse_lte_mac_subpkt_v1_ul_transport_block(pkt_header, subpkt_mac, subpkt_body, args)
                mac_pkts += ul_ret
                kpi_lines += ul_kpi
                n = ul_extra.get('ul_bytes', 0) or 0
                if is_dl_log:
                    result_ul_bytes += n
                else:
                    result_dl_bytes += n
                result_ul_harq_ndi.extend(ul_extra.get('ul_harq_ndi', []))
                result_ul_mcs_sum += ul_extra.get('ul_mcs_sum', 0)
                result_ul_mcs_count += ul_extra.get('ul_mcs_count', 0)
            elif subpkt_mac.id == 0x00:
                # Subpacket 0x00: common/padding, skip silently
                pass
            else:
                if self.parent:
                    if subpkt_mac.id not in self.parent._logged_unhandled_mac_subpkt_ids:
                        self.parent._logged_unhandled_mac_subpkt_ids.add(subpkt_mac.id)
                        # Log at DEBUG only (0x03 etc. common in RACH/9607; use --debug to see)
                        self.parent.logger.log(logging.DEBUG, 'Unhandled LTE MAC Subpacket ID 0x{:02x}'.format(subpkt_mac.id))
                        self.parent.logger.log(logging.DEBUG, util.xxd(pkt_body))
                continue

        if len(mac_pkts) > 0 or kpi_lines or result_dl_bytes or result_ul_bytes:
            result = {'ts': pkt_ts}
            if kpi_lines:
                result['stdout'] = '\n'.join(kpi_lines)
            if len(mac_pkts) > 0:
                ...
                result['cp'] = mac_pkts
            if result_dl_bytes:
                result['dl_bytes'] = result_dl_bytes
            if result_ul_bytes:
                result['ul_bytes'] = result_ul_bytes
            if result_ul_harq_ndi:
                result['ul_harq_ndi'] = result_ul_harq_ndi
            if result_ul_mcs_count > 0:
                result['ul_mcs_sum'] = result_ul_mcs_sum
                result['ul_mcs_count'] = result_ul_mcs_count
            return result
        return None
    ...
    def parse_lte_mac_subpkt_v49(self, pkt_header, pkt_body: bytes, args: dict, is_downlink: bool):
        ...
        radio_id = (self.parent.sanitize_radio_id(args.get('radio_id', 0)) if self.parent and args else 0)
        ...
        for i in range(num_tb):
            ...
            if is_downlink:
                total_dl_bytes += subpkt_tb.size
            else:
                total_ul_bytes += subpkt_tb.size
        ...
        if len(mac_pkts) > 0 or kpi_lines or total_dl_bytes or total_ul_bytes:
            result = {'layer': 'mac', 'cp': mac_pkts, 'ts': pkt_ts}
            if kpi_lines:
                result['stdout'] = '\n'.join(kpi_lines)
            if total_dl_bytes:
                result['dl_bytes'] = total_dl_bytes
            if total_ul_bytes:
                result['ul_bytes'] = total_ul_bytes
            return result
        return None

    def parse_lte_mac_rach_trigger(self, pkt_header, pkt_body: bytes, args: dict):
        # 0x01/0x31/0x32 = v1 subpacket layout; try any version so 9607 and others still get RACH KPI
        if len(pkt_body) < 4:
            return None
        try:
            if pkt_body[1] <= 32:  # num_subpacket plausible
                return self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args)
        except Exception:
            pass
        if self.parent and pkt_body[0] not in (0x01, 0x31, 0x32):
            self.parent.logger.log(logging.DEBUG, 'LTE MAC RACH trigger version 0x{:02x} not parsed'.format(pkt_body[0]))
        return None
    ...
    def parse_lte_mac_rach_response(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        try:
            ...
            pass
        if self.parent and pkt_body[0] not in (0x01, 0x31, 0x32):
            self.parent.logger.log(logging.DEBUG, 'LTE MAC RACH response version 0x{:02x} not parsed'.format(pkt_body[0]))
        return None

    def parse_lte_mac_9607_dl_safe(self, pkt_header, pkt_body: bytes, args: dict):
        """Safe wrapper for 0xB167/0xB169 (9607 MAC-style). Never raises; returns None on any error."""
        try:
            args_9607 = dict(args) if args else {}
            args_9607['_from_9607'] = True
            return self.parse_lte_mac_dl_block(pkt_header, pkt_body, args_9607)
        except Exception:
            return None

    def parse_lte_mac_9607_ul_safe(self, pkt_header, pkt_body: bytes, args: dict):
        """Safe wrapper for 0xB168/0xB16A (9607 MAC-style). Never raises; returns None on any error."""
        try:
            args_9607 = dict(args) if args else {}
            args_9607['_from_9607'] = True
            return self.parse_lte_mac_ul_block(pkt_header, pkt_body, args_9607)
        except Exception:
            return None
    ...
    def parse_lte_mac_dl_block(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        if pkt_version == 0x01:
            return self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args, is_dl_log=True)
        elif pkt_version == 0x05:
            # 9607 and some modems use 0xB167 etc. with version 0x05; format usually differs from v1
            if len(pkt_body) >= 2 and pkt_body[1] <= 20:
                try:
                    v1_result = self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args, is_dl_log=True)
                    if v1_result is not None and (v1_result.get('stdout') or v1_result.get('cp')):
                        return v1_result
                except Exception:
                    pass
            return None
        elif pkt_version == 0x31 or pkt_version == 0x32:
            # Some modems use v1 subpacket layout with version 0x31/0x32; try v1 first for KPI (TA, DL MCS)
            try:
                v1_result = self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args, is_dl_log=True)
                if v1_result is not None and (v1_result.get('stdout') or v1_result.get('cp')):
                    return v1_result
            ...
            # 9607 may use other versions (e.g. 0x02, 0x03); try v1 if body looks like v1 (num_subpacket in 1..20)
            if len(pkt_body) >= 2 and 1 <= pkt_body[1] <= 20:
                try:
                    if self.parent:
                        self.parent.logger.log(logging.DEBUG, 'MAC DL: trying v1 parser for unknown version 0x{:02x}'.format(pkt_version))
                    v1_result = self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args, is_dl_log=True)
    ...
    def parse_lte_mac_ul_block(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        if pkt_version == 0x01:
            return self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args, is_dl_log=False)
        elif pkt_version == 0x05 or pkt_version == 0x31 or pkt_version == 0x32:
            # 9607 / some modems use 0xB168 etc.; try v1 first, then v49 for throughput/UL MCS
            if len(pkt_body) >= 2 and pkt_body[1] <= 20:
                try:
                    v1_result = self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args, is_dl_log=False)
                    if v1_result is not None:
                        return v1_result
                except Exception:
                    pass
            try:
                v49_result = self.parse_lte_mac_subpkt_v49(pkt_header, pkt_body, args, False)
                if v49_result is not None and (v49_result.get('stdout') or v49_result.get('ul_bytes')):
                    return v49_result
            except Exception:
                pass
            if self.parent:
            ...
            # 9607 may use other versions; try v1 if body looks like v1 (num_subpacket in 1..20)
            if len(pkt_body) >= 2 and 1 <= pkt_body[1] <= 20:
                try:
                    if self.parent:
                        self.parent.logger.log(logging.DEBUG, 'MAC UL: trying v1 parser for unknown version 0x{:02x}'.format(pkt_version))
                    v1_result = self.parse_lte_mac_subpkt_v1(pkt_header, pkt_body, args, is_dl_log=False)
                    if v1_result is not None:
                        return v1_result
                except Exception:
    ...
    def parse_lte_mib(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        stdout = ''
        if isinstance(item, item_struct) and self.parent:
            radio_id = args['radio_id'] if args and 'radio_id' in args else 0
            bw = item.bandwidth
            if bw in (0, 1, 2, 3, 4, 5):
                bw = (6, 15, 25, 50, 75, 100)[bw]
            self.parent.lte_last_bw_dl[radio_id] = bw
    ...
    def parse_lte_rrc(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        rrc_channel = getattr(gsmtap_subtype[1], 'name', str(gsmtap_subtype[1]))
