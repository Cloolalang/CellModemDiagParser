...
class DiagLteLogParser:
    ...
    def parse_lte_mac_subpkt_v1_rach_attempt(self, pkt_header, subpkt_hdr, subpkt_body: bytes) -> tuple[list[bytes], str | None]:
        ...
        # Build KPI line (success or failure)
        ...
        ta_str = str(rach_msg2.ta) if rach_msg2 is not None else '-'
        ...
        kpi_line = 'LTE KPI RACH: result={}, attempt={}, contention={}, preamble={}, preamble_power_dB={}, ta={}, tc_rnti={}, earfcn={}'.format(
            result_str, subpkt_mac_rach_attempt.num_attempt, subpkt_mac_rach_attempt.contention,
            preamble_str, preamble_power_str, ta_str, tc_rnti_str, earfcn_str)
        ...
        for j in range(n_samples):
            ...
            if isinstance(subpkt_mac_dl_tb, subpkt_mac_dl_tb_struct) or isinstance(subpkt_mac_dl_tb, subpkt_mac_dl_tb_struct_v4):
                ...
                if self.parent and getattr(self.parent, 'show_kpi', False):
                    ...
                    n_prb = self.parent.lte_last_bw_dl[radio_id] if self.parent else 0
                    mcs = lte_tbs.tbs_to_mcs(subpkt_mac_dl_tb.dl_tbs, n_prb) if n_prb else None
                    if mcs is not None:
                        prb_to_mhz = {6: 1.4, 15: 3, 25: 5, 50: 10, 75: 15, 100: 20}
                        bw_mhz = prb_to_mhz.get(n_prb, n_prb)
                        if bw_mhz == int(bw_mhz):
                            kpi_lines.append('{}MHz BW MCS={}'.format(int(bw_mhz), mcs))
                        else:
                            kpi_lines.append('{:.1f}MHz BW MCS={}'.format(bw_mhz, mcs))
                    # Parse MAC PDU for Timing Advance command (LCID 0x1D per 36.321)
                    for i in range(len(mac_hdr) - 1):
                        if (mac_hdr[i] & 0x1F) == 0x1D:  # TA command LCID
                            ta = mac_hdr[i + 1] & 0x3F  # 6 bits TA (0-63)
                            self.parent.lte_last_ta[radio_id] = ta
                            kpi_lines.append('LTE KPI: TA={}'.format(ta))
                            break
    ...
    def parse_lte_mac_subpkt_v1_ul_transport_block(self, pkt_header, subpkt_hdr, subpkt_body: bytes, args: dict = None) -> tuple[list[bytes], list[str], dict]:
        ...
        for j in range(n_samples):
            ...
            if isinstance(subpkt_mac_ul_tb, subpkt_mac_ul_tb_struct) or isinstance(subpkt_mac_ul_tb, subpkt_mac_ul_tb_struct_v2):
                ul_mcs = subpkt_mac_ul_tb.grant & 0x1F
                radio_id = args['radio_id'] if args and 'radio_id' in args else 0
                ...
                if self.parent and getattr(self.parent, 'show_kpi', False):
                    kpi_lines.append('LTE KPI UL: MCS={}'.format(ul_mcs))
        ...
        extra = {'ul_bytes': total_ul_bytes} if total_ul_bytes else {}
        return ret, kpi_lines, extra
    ...
    def parse_lte_mac_rach_trigger(self, pkt_header, pkt_body: bytes, args: dict):
        ...
        # 0x01 = v1 (subpackets); 0x31/0x32 = same subpacket layout on many devices
        if pkt_version == 0x01 or pkt_version == 0x31 or pkt_version == 0x32:
