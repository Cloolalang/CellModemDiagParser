...
class QualcommParser(AbstractParser):
    def __init__(self):
        ...
        self._logged_unknown_ml1_scell_versions: set[int] = set()
        self._logged_skip_diag_log_ids: set[int] = set()
        self._logged_not_parsing_diag_log_ids: set[int] = set()
        self._logged_unhandled_mac_subpkt_ids: set[int] = set()
        ...
        self._last_neighbor_cell_lines = {}  # radio_id -> list of "Neighbor cell N: ..." lines (cleared on new Primary Cell)
        ...
        self._ul_ndi_per_harq = {}  # radio_id -> { harq_id: last_ndi } for ACK/NACK inference
        self._ul_tb_new = {}  # radio_id -> count of new UL TB (NDI toggled => eNodeB ACK)
        self._ul_tb_retx = {}  # radio_id -> count of UL retx (NDI same => eNodeB NACK)
        self._ul_mcs_sum = {}  # radio_id -> sum of UL MCS in throughput window (for avg MCS = uplink channel quality)
        self._ul_mcs_count = {}  # radio_id -> count of UL TB in window
        self.ul_mcs_invert = False  # if True: display UL avg MCS as (28 - avg); use when MCS goes down as path loss improves
        self.ul_ndi_bit = 6  # bit index in 16-bit grant for NDI (0-15); modem-dependent, try 10 if retx rate looks wrong
        self.ul_ndi_invert = False  # if True: same NDI => new (ACK), toggled => retx (NACK); for modems with inverted NDI semantics
        self.show_ul_retx_pct = True  # if False: do not append (UL retransmit: X%) to throughput line (use when modem NDI is unreliable)
        ...
        for p in params:
            if p == 'log_level':
                ...
                if b is not None and 0 <= b <= 15:
                    self.ul_ndi_bit = int(b)
            elif p == 'invert-ul-ndi':
                self.ul_ndi_invert = bool(params[p])
            elif p == 'invert-ul-mcs':
                self.ul_mcs_invert = bool(params[p])
            elif p == 'no-ul-retransmit':
                self.show_ul_retx_pct = not bool(params[p])
            elif p == 'layer':
    ...
    def run_diag(self, writer: AbstractWriter | None = None) -> None:
        ...
        self._logged_unknown_ml1_scell_versions.clear()
        self._logged_skip_diag_log_ids.clear()
        self._logged_not_parsing_diag_log_ids.clear()
        self._logged_unhandled_mac_subpkt_ids.clear()
        try:
            while loop:
                ...
                try:
                    ...
                    reopen_count += 1
                    if reopen_count <= max_reopen and getattr(self.io_device, 'reopen', None) is not None:
                        self.logger.warning(
                            'Serial/COM port error during run (e.g. EPS download): %s. Reopening port (attempt %d/%d) ...',
                            e, reopen_count, max_reopen
                        )
                        try:
                            self.io_device.reopen()
                        except Exception as reopen_err:
                            self.logger.error('Reopen failed: %s', reopen_err)
                        time.sleep(2)  # give OS/modem time to release port before next attempt
                        continue
                    self.logger.error(
    ...
    def _log_line_to_json(self, radio_id: int, line: str, ts: datetime.datetime) -> dict:
        ...
        # RACH KPI: preamble, preamble_power_dB, ta, tc_rnti, earfcn; optional p_max, backoff
        m = re.match(r'LTE KPI RACH:\s*result=(\w+),\s*attempt=(\d+),\s*contention=(\d+),\s*preamble=([-\d]+),\s*preamble_power_dB=([-\d]+),\s*ta=([-\d]+),\s*tc_rnti=(0x[0-9a-fA-F]+|-),\s*earfcn=([-\d]+)(?:,\s*p_max=([-\d]+))?(?:,\s*backoff=([-\d]+))?', line)
        if m:
            def _opt_int(s):
                return None if s is None or s == '-' else int(s)
            tc_rnti_val = None if m.group(7) == '-' else int(m.group(7), 16)
            preamble_power = _opt_int(m.group(5))
            out = {
                'type': 'lte_rach',
                'result': m.group(1),
                ...
                'preamble_power_dB': preamble_power,
                'preamble_power_dbm': preamble_power,
                'ta': _opt_int(m.group(6)),
                'tc_rnti': tc_rnti_val,
                'earfcn': _opt_int(m.group(8))
            }
            if m.lastindex >= 9 and m.group(9) is not None:
                out['p_max'] = _opt_int(m.group(9))
            if m.lastindex >= 10 and m.group(10) is not None:
                out['backoff'] = _opt_int(m.group(10))
            base.update(out)
            return base

        m = re.match(r'LTE throughput: ([\d.]+) Mbps(?: \[UL avg MCS: ([\d.]+)\])?(?: \(UL retransmit: ([\d.]+)%\))?', line)
        if m:
            obj = {'type': 'lte_throughput', 'mbps': float(m.group(1))}
            if m.group(2) is not None:
                obj['ul_avg_mcs'] = float(m.group(2))
            if m.group(3) is not None:
                obj['ul_retx_pct'] = float(m.group(3))
            base.update(obj)
            return base
        ...
        m = re.match(r'LTE RRC NEW_CELL_IND: EARFCN=(\d+), PCI=(\d+)', line)
        if m:
            base.update({
                'type': 'lte_rrc_new_cell_ind',
                'earfcn': int(m.group(1)),
                'pci': int(m.group(2))
            })
            return base

        m = re.match(r'LTE RRC NEW_CELL_IND: (.+)', line)
        if m:
            base.update({'type': 'lte_rrc_new_cell_ind', 'raw': m.group(1).strip()})
            return base
    ...
    def postprocess_parse_result(self, parse_result: dict[str, Any]):
        ...
        # UL HARQ NDI: infer eNodeB ACK/NACK from NDI toggle (toggled => ACK/new TX, same => NACK/retx)
        if self.show_kpi and 'ul_harq_ndi' in parse_result:
            if radio_id not in self._ul_ndi_per_harq:
                self._ul_ndi_per_harq[radio_id] = {}
            if radio_id not in self._ul_tb_new:
                self._ul_tb_new[radio_id] = 0
            if radio_id not in self._ul_tb_retx:
                self._ul_tb_retx[radio_id] = 0
            invert = getattr(self, 'ul_ndi_invert', False)
            for (harq_id, ndi) in parse_result['ul_harq_ndi']:
                last_ndi = self._ul_ndi_per_harq[radio_id].get(harq_id)
                self._ul_ndi_per_harq[radio_id][harq_id] = ndi
                if last_ndi is not None:
                    same_ndi = (ndi == last_ndi)
                    if invert:
                        # Modem uses inverted NDI: same => new (ACK), toggled => retx (NACK)
                        if same_ndi:
                            self._ul_tb_new[radio_id] += 1
                        else:
                            self._ul_tb_retx[radio_id] += 1
                    else:
                        if same_ndi:
                            self._ul_tb_retx[radio_id] += 1
                        else:
        ...
        # UL MCS: accumulate for avg MCS (uplink channel quality; higher MCS = better UL channel)
        if self.show_kpi and 'ul_mcs_sum' in parse_result and 'ul_mcs_count' in parse_result:
            self._ul_mcs_sum[radio_id] = self._ul_mcs_sum.get(radio_id, 0) + parse_result.get('ul_mcs_sum', 0)
            self._ul_mcs_count[radio_id] = self._ul_mcs_count.get(radio_id, 0) + parse_result.get('ul_mcs_count', 0)
        ...
        if self.show_kpi and ('dl_bytes' in parse_result or 'ul_bytes' in parse_result):
            ...
            # Throttle throughput report to at most every 5 seconds
            if time.time() - self._throughput_window_start[radio_id] >= 5.0:
                if self._lte_rrc_state.get(radio_id) == 'RRC_CONNECTED':
                    ...
                    ul_mcs_cnt = self._ul_mcs_count.get(radio_id, 0)
                    if ul_mcs_cnt > 0:
                        ul_avg_mcs = self._ul_mcs_sum.get(radio_id, 0) / ul_mcs_cnt
                        if getattr(self, 'ul_mcs_invert', False):
                            ul_avg_mcs = 28.0 - ul_avg_mcs
                        line += ' [UL avg MCS: {:.1f}]'.format(ul_avg_mcs)
                    if getattr(self, 'show_ul_retx_pct', True):
                        ul_new = self._ul_tb_new.get(radio_id, 0)
                        ul_retx = self._ul_tb_retx.get(radio_id, 0)
                        ul_total = ul_new + ul_retx
                        if ul_total > 0:
                            ul_retx_pct = 100.0 * ul_retx / ul_total
                            line += ' (UL retransmit: {:.1f}%)'.format(ul_retx_pct)
                ...
                self._ul_tb_new[radio_id] = 0
                self._ul_tb_retx[radio_id] = 0
                self._ul_mcs_sum[radio_id] = 0
                self._ul_mcs_count[radio_id] = 0
                ...
                if self.combine_stdout:
                    ...
                    for l in parse_result['stdout'].split('\n'):
                        ...
                        if self.show_kpi and not l.startswith('LTE KPI RACH:') and l == self._last_kpi_line.get(radio_id):
                            continue
                        # Throttle "LTE Primary Cell: EARFCN: ..." (avoid pairs from 0xB17F + 0xB193); same interval idle/connected so DL KPI continues when connected
                        if self.show_kpi and l.startswith('LTE Primary Cell: EARFCN:'):
                            interval = 1.0
                            if (time.time() - self._last_scell_emit_time.get(radio_id, 0)) < interval:
                        ...
                        # Use modem-reported bandwidth from SCell Info so "X MHz BW MCS" shows correct carrier (e.g. 10 MHz)
                        scell_bw_m = re.search(r'LTE RRC SCell Info: .* Bandwidth: ([\d.]+)/[\d.]+ MHz', l)
                        if scell_bw_m:
                            try:
                                mhz = float(scell_bw_m.group(1))
                                mhz_to_prb = {1.4: 6, 3: 15, 5: 25, 10: 50, 15: 75, 20: 100}
                                prb = mhz_to_prb.get(mhz)
                                if prb is not None and radio_id < len(self.lte_last_bw_dl):
                                    self.lte_last_bw_dl[radio_id] = prb
                            except (ValueError, IndexError):
                                pass
                        # Track RRC state and last SCell for re-emitting in connected state
                        rrc_m = re.match(r'LTE RRC State: (.+)', l)
                        if rrc_m:
                            new_state = rrc_m.group(1).strip()
                            self._lte_rrc_state[radio_id] = new_state
                        if l.startswith('LTE Primary Cell: EARFCN:') or l.startswith('LTE Primary Cell (Connected):'):
                            self._last_scell_line[radio_id] = l
                            self._last_neighbor_cell_lines[radio_id] = []  # new Primary Cell, clear neighbor list
                            if not (self.show_kpi and l.startswith('LTE Primary Cell: EARFCN:')):
                                self._last_scell_emit_time[radio_id] = time.time()
                        if l.startswith('Neighbor cell '):
                            if radio_id not in self._last_neighbor_cell_lines:
                                self._last_neighbor_cell_lines[radio_id] = []
                            self._last_neighbor_cell_lines[radio_id].append(l)
                        # In RRC_CONNECTED modem often stops sending DL KPI; re-emit last Primary Cell + Neighbor cells periodically so DL KPI stays visible
                        if (self.show_kpi and self._lte_rrc_state.get(radio_id) == 'RRC_CONNECTED'
                            ...
                            for nb_line in self._last_neighbor_cell_lines.get(radio_id, []):
                                print('Radio {}: {}'.format(radio_id, nb_line))
                                self._send_json_udp(radio_id, nb_line, ts)
                            self._last_scell_emit_time[radio_id] = time.time()
                        if self.show_kpi:
                            self._last_kpi_line[radio_id] = l
    ...
    def parse_diag_log(self, pkt: bytes, args: dict[str, Any] | None=None):
        ...
        if pkt_header.log_id in self.process.keys():
            ...
            if pkt_header.log_id not in self._logged_skip_diag_log_ids:
                self._logged_skip_diag_log_ids.add(pkt_header.log_id)
                self.logger.log(logging.DEBUG, 'Skip processing DIAG log item {:#06x}'.format(pkt_header.log_id))
            return None
        else:
            if pkt_header.log_id not in self._logged_not_parsing_diag_log_ids:
                self._logged_not_parsing_diag_log_ids.add(pkt_header.log_id)
