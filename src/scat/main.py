...
def scat_main():
    ...
    if 'qc' in parser_dict.keys():
        ...
        qc_group.add_argument('--invert-tx-power', action='store_true', help='Invert TX power display so good signal shows lower TX and bad signal higher (matches power-control intuition when modem PHR looks opposite)')
    ...
    if args.type == 'qc':
        ...
        current_parser.set_parameter({
            ...
            'invert-tx-power': getattr(args, 'invert_tx_power', False),
            'layer': layers,
            'format': args.format,
