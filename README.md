...
This application parses diagnostic messages of Qualcomm and Samsung baseband (from diagnostic interfaces such as USB or serial), and generates a stream of GSMTAP packets containing cellular control plane messages. This fork additionally provides more detailed KPIs via application log messages and via UDP as JSON.
...
* **KPI output** (`--kpi`): Serving and neighbor cell (RSRP, RSRQ, RSSI, PCI, EARFCN, priority), RRC state (including RRC_SEARCH, RRC_INACTIVE), combined throughput (from MAC TB sizes), **UL avg MCS** (uplink channel quality), optional UL retransmit % (NDI-based), **LTE Timing Advance** (event 1498), RACH result (preamble, power, TA, tc_rnti). Options: `--invert-ul-mcs`, `--ul-ndi-bit`, `--invert-ul-ndi`, `--no-ul-retransmit`.
* **JSON over UDP** (`--json-udp-port`): Each KPI line is sent as a structured JSON packet to localhost (e.g. for Node-RED). Types include `lte_primary_cell`, `lte_rrc_state`, `lte_rach`, `rrc_event`, `lte_throughput` (with `ul_avg_mcs`, `ul_retx_pct` when applicable), and others (see [KPI_USAGE.md](KPI_USAGE.md)).
...
* **`--kpi`** — Show LTE KPIs in the log: serving cell (EARFCN, PCI, RSRP, RSSI, RSRQ), neighbor cells, RRC state, throughput, UL avg MCS, optional UL retransmit %, Timing Advance, RACH. In RRC_CONNECTED, if the modem does not report serving cell, the last known SCell is re-printed at most every 2 seconds so you still see EARFCN/PCI/RSRP.
* **`--dl-bandwidth`** — DL bandwidth in MHz (1.4, 3, 5, 10, 15, or 20) for MCS lookup when cell info is not yet available.
* **`--json-udp-port PORT`** — Send each log line as JSON over UDP to `127.0.0.1:PORT` (e.g. for automation or dashboards).
...
- **UL avg MCS** — Average uplink MCS (0–28) of UL grants in that window. Higher = better UL channel quality. Use `--invert-ul-mcs` if your modem reports MCS inversely (value goes down as path loss improves).
- **UL retransmit %** — Inferred from NDI in the UL grant (modem-dependent; can be unreliable). Use `--no-ul-retransmit` to hide it.

### UL metrics (Qualcomm)

| Option | Description |
|--------|-------------|
| `--ul-ndi-bit BIT` | Bit index (0–15) in UL grant for NDI. Default `6`. Try `5` or `10` if retransmit % looks wrong. |
...
See **[KPI_USAGE.md](KPI_USAGE.md)** for full KPI output description, JSON types, and notes.

### Example

```bash
python -m scat -t qc -s COM14 --kpi --dl-bandwidth 20 --json-udp-port 9999 --disable-crc-check --invert-ul-mcs
```

GSMTAP control-plane packets use `-H` (host) and `-P` (port); see the [SCAT repository](https://github.com/fgsect/scat) for defaults.
